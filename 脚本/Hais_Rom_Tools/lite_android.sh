...
PROJECT=../Rom/MIUI
ROM_BASE=$PROJECT/UnpackedImg
ROM_SYSTEM=$ROM_BASE/system
ROM_VENDOR=$ROM_BASE/vendor

for FILE in $DELETE_SYSTEM ; do
  echo "- 删除System文件: "$FILE
  rm -rf $ROM_SYSTEM$FILE
done
for FILE in $DELETE_VENDOR ; do
  echo "- 删除Vendor文件: "$FILE
...
function repairXbinFsAndCtx(){
	for file in `ls ./${1}_files/xbin`
		do
			echo "- 修复 $ROM_BASE/${1}/xbin/$file"
			echo "${1}/xbin/${file} 0000 2000 00755" >> $FS_PATH
			echo "/${1}/xbin/${file} u:object_r:${file}_exec:s0" >> $CTX_PATH
		done
}
...
function repairThemeFsAndCtx(){
	for file in `ls ./${1}_files/media/theme/default`
		do
			echo "- 修复 $ROM_BASE/${1}/media/theme/default/$file"
			echo "${1}/media/theme/default/${file} 0000 0000 00644" >> $FS_PATH
			echo "/${1}/media/theme/default/${file} u:object_r:system_file:s0" >> $CTX_PATH
		done
}
